...
train, test = train_test_split(train_df, train_size=0.8, random_state=1234)
...
for i in np.arange(0.1, 5.1, 0.1):
        ...
        X_train = train1[train_features]
        y_train = train1.logerror
        model.fit(X_train, y_train, cat_features=cat_feature_inds)
        y_pred = model.predict(X_test)
        res = np.mean(abs(y_pred - y_test))
        print(i , res)
        mae.append(res)
    
import matplotlib.pyplot as plt
plt.plot(np.arange(0.1, 5.1, 0.1), mae)

"""
Conclusion:
    - no capping is necessary for catboost
"""