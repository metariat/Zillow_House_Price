...
density = fread("C:/documents/xq.do/Desktop/Kaggle/Zillow_House_Price/Excel files/parcel_density.csv")
density[, longitude := NULL]
density[, latitude := NULL]
...
train = merge(train, density, by = "parcelid", all.x = T)
test = merge(test, density, by = "parcelid", all.x = T)
gc()
...
#Prediction
gc()
y.hat = predict(cb, x.test)
mean(abs(y.hat - y.test))
#0.06630337
